...
void FsLazyWindowApplication::RemakeVertexArray(void)
{
	...
	for(auto plHd=mesh.NullPolygon(); YSTRUE==mesh.MoveToNextPolygon(plHd); )
	{
...
YsShellExt::VertexHandle FsLazyWindowApplication::PickedVtHd(int mx,int my,int pixRange) const
{
	...
	for(auto vtHd=mesh.NullVertex(); YSTRUE ==mesh.MoveToNextVertex(vtHd); )
	{
...
/* virtual */ void FsLazyWindowApplication::Initialize(int argc,char *argv[])
{
	mesh.EnableSearch();
	mode = 0;
	if(2<=argc && YSTRUE ==mesh.LoadStl(argv[1]))
	{
